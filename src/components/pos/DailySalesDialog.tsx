...
const DailySalesDialog: React.FC<DailySalesDialogProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <>
            ...
            <DialogContent 
                className="!flex !flex-col !p-0 !overflow-hidden !max-w-[95vw] sm:!max-w-4xl lg:!max-w-6xl w-full !h-[90dvh] !max-h-[90dvh] bg-background border-border !rounded-[2rem] shadow-2xl"
                centerOnMobile={true}
            >
                    {/* Header Section */}
                    <div className="p-4 sm:p-6 border-b border-border bg-transparent flex flex-col gap-3 sm:gap-4 [@media(max-height:580px)]:p-2 [@media(max-height:580px)]:gap-2 shrink-0">
                        <div className="flex justify-between items-center">
                            <div className="space-y-0.5">
                                <DialogTitle className="text-base sm:text-xl font-bold text-foreground flex items-center gap-2">
                                    <ReceiptText className="h-4 sm:h-5 w-4 sm:w-5 text-emerald-500" />
                                    Historial de Facturación
                                </DialogTitle>
                                <DialogDescription className="text-muted-foreground text-[9px] sm:text-[10px] font-medium [@media(max-height:580px)]:hidden">
                        ...
                        <div className="grid grid-cols-3 gap-2 sm:gap-3 [@media(max-height:580px)]:hidden">
                            <div className="bg-muted/50 border border-border p-2 sm:p-4 rounded-xl transition-all hover:border-muted-foreground/40">
                                <div className="flex items-center justify-between mb-0.5 sm:mb-1">
                                    <span className="text-[8px] sm:text-[10px] font-semibold text-muted-foreground uppercase tracking-wider truncate">
                                        <span className="inline sm:hidden">Facturas</span>
                                        <span className="hidden sm:inline">Facturas Totales</span>
                                    </span>
                                    <Hash className="hidden sm:block h-3.5 w-3.5 text-muted-foreground" />
                                </div>
                                <div className="text-base sm:text-2xl font-black text-foreground leading-none">{totals.salesCount}</div>
                                <div className="mt-0.5 sm:mt-1 text-[7px] sm:text-[8px] text-muted-foreground font-bold tracking-wide uppercase truncate">Comprobantes</div>
                            </div>

                            <div className="bg-muted/50 border border-border p-2 sm:p-4 rounded-xl transition-all hover:border-emerald-500/20">
                                <div className="flex items-center justify-between mb-0.5 sm:mb-1">
                                    <span className="text-[8px] sm:text-[10px] font-semibold text-muted-foreground uppercase tracking-wider truncate">
                                ...
                                <div className="text-base sm:text-2xl font-black text-foreground leading-none truncate">
                            ...
                            <div className="bg-muted/50 border border-border p-2 sm:p-4 rounded-xl transition-all hover:border-blue-500/20">
                                <div className="flex items-center justify-between mb-0.5 sm:mb-1">
                                    <span className="text-[8px] sm:text-[10px] font-semibold text-muted-foreground uppercase tracking-wider truncate">
                                ...
                                <div className="mt-1 flex items-center gap-1 overflow-x-auto no-scrollbar">
                                    <span className="text-[6.5px] sm:text-[8px] font-extrabold text-muted-foreground bg-muted px-1 py-0.2 sm:py-0.5 rounded shrink-0">INI: {totals.initialCash.toLocaleString('es-DO', { maximumFractionDigits: 0 })}</span>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="relative flex-1 group">
                                <Search className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 h-3.5 sm:h-4 w-3.5 sm:w-4 text-muted-foreground group-focus-within:text-emerald-500 transition-colors" />
                                <Input
                                    placeholder="Buscar factura o cliente..."
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                    className="h-10 pl-9 sm:pl-11 bg-muted border-border rounded-lg text-foreground placeholder:text-muted-foreground focus:ring-1 focus:ring-emerald-500/30 text-xs sm:text-sm"
                                />
                            </div>
                            <div className="flex items-center gap-1.5 shrink-0">
                                {profile?.role === 'admin' && (
                                    <Select value={userFilter} onValueChange={setUserFilter}>
                                        <SelectTrigger className="w-[110px] sm:w-[180px] h-10 bg-muted border-border rounded-lg text-foreground font-semibold text-xs">
                                            <div className="flex items-center gap-1.5 min-w-0">
                                                <User className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                                                <span className="truncate"><SelectValue placeholder="Usuario" /></span>
                                            </div>
                                        </SelectTrigger>
                                        <SelectContent className="bg-muted border-border text-foreground p-1 rounded-xl">
                                ...
                                <Button 
                                    variant="outline" 
                                    className="h-10 px-3 sm:px-4 rounded-lg border-border bg-muted hover:bg-accent text-foreground font-bold text-xs tracking-wide flex items-center justify-center shrink-0"
                                >
                    ...
                    <div className="flex-1 overflow-hidden px-3 sm:px-6 pb-3 sm:pb-6 flex flex-col min-h-0 [@media(max-height:580px)]:px-2 [@media(max-height:580px)]:pb-2">
                        <div className="flex-1 bg-transparent border border-border/40 rounded-2xl overflow-hidden flex flex-col mt-2 min-h-0 [@media(max-height:580px)]:mt-1">
                            <div className="flex-1 overflow-y-auto no-scrollbar min-h-0">
                                {isLoading ? (
                                    <div className="flex flex-col items-center justify-center h-full gap-4 opacity-50">
                                        <div className="h-8 w-8 rounded-full border-2 border-border border-t-emerald-500 animate-spin" />
                                        <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Cargando transacciones...</p>
                                    </div>
                                    ...
                                    <div className="flex flex-col items-center justify-center h-full text-center p-6 space-y-3">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-muted-foreground font-semibold text-sm">No se encontraron facturas</p>
                                            <p className="text-muted-foreground text-xs max-w-xs">Aún no hay transacciones registradas en este turno o no coinciden con los filtros.</p>
                                        </div>
                                    </div>
                                ) : (
                                    <>
                                            ...
                                            <Table>
                                                <TableHeader className="bg-muted/30 backdrop-blur-md sticky top-0 z-10">
                                                    <TableRow className="border-border hover:bg-transparent">
                                                        <TableHead className="py-2.5 px-4 text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Factura</TableHead>
                                                        <TableHead className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Cliente / Cajero</TableHead>
                                                        <TableHead className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Pago</TableHead>
                                                        <TableHead className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider text-right">Total</TableHead>
                                                        <TableHead className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider text-center w-32">Acciones</TableHead>
                                                    </TableRow>
                                                </TableHeader>
                                                <TableBody>
                                                    {filteredSales.map((sale) => (
                                                        <TableRow key={sale.id} className="border-border hover:bg-muted/30 transition-colors cursor-pointer" onDoubleClick={() => handlePreview(sale)}>
                                                            <TableCell className="py-3 px-4">
                                                                 <div className="flex flex-col">
                                                                     <span className="text-xs font-bold text-foreground tracking-tight">{sale.invoice_number}</span>
                                                            ...
                                                            <TableCell className="py-3">
                                                                <div className="flex flex-col">
                                                                    <span className="text-xs font-semibold text-foreground">{sale.customer?.name || 'Venta Rápida'}</span>
                                                                    <span className="text-[9px] text-muted-foreground font-bold uppercase tracking-tight">{sale.profile?.full_name || 'Sistema'}</span>
                                                                </div>
                                                            </TableCell>
                                                            <TableCell className="py-3">
                                                                <div className={cn(
                                                            ...
                                                            <TableCell className={cn(
                                                                ...
                                                                sale.total < 0 ? "text-red-500" : "text-foreground"
                                                            )}>
                                                            ...
                                                            <TableCell className="px-4 py-3">
                                                                <div className="flex items-center justify-center gap-1.5">
                                                                    ...
                                                                    <Button
                                                                        ...
                                                                        className="h-8 w-8 rounded-lg hover:bg-emerald-500/10 hover:text-emerald-500 text-muted-foreground transition-colors"
                                                                        title="Imprimir"
                                                                    >
                                                                    ...
                                                                    <Button
                                                                        ...
                                                                        className="h-8 w-8 rounded-lg hover:bg-blue-500/10 hover:text-blue-500 text-muted-foreground transition-colors"
                                                                        title="Correo"
                                                                    >
                                                                    ...
                                                                    <Button
                                                                        ...
                                                                        className="h-8 w-8 rounded-lg hover:bg-muted text-muted-foreground transition-colors"
                                                                        title="Descargar"
                                                                    >
                                        ...
                                        <div className="block sm:hidden space-y-2 p-1.5">
                                            {filteredSales.map((sale) => (
                                                <div 
                                                    key={sale.id}
                                                    onClick={() => handlePreview(sale)}
                                                    className="bg-muted/40 border border-border rounded-xl p-3.5 hover:border-border transition-colors cursor-pointer"
                                                >
                                                    {/* Top Row: Invoice Number & Total */}
                                                    <div className="flex justify-between items-start gap-2 mb-2">
                                                        <div className="flex flex-col">
                                                            <span className="text-xs font-black text-foreground tracking-tight">{sale.invoice_number}</span>
                                                            <span className="text-[9px] text-muted-foreground font-bold uppercase tracking-tight mt-0.5">{format(new Date(sale.created_at), 'hh:mm a')}</span>
                                                        </div>
                                                        <span className={cn(
                                                    ...
                                                    <div className="flex justify-between items-center gap-2 mb-3">
                                                        <div className="flex flex-col min-w-0 flex-1">
                                                            <span className="text-[11px] font-bold text-foreground truncate">{sale.customer?.name || 'Venta Rápida'}</span>
                                                            <span className="text-[9px] text-muted-foreground font-bold uppercase tracking-tight mt-0.5 truncate">{sale.profile?.full_name || 'Sistema'}</span>
                                                        </div>
                                                        <span className={cn(
                                                    ...
                                                    <div className="flex items-center justify-end gap-1.5 pt-2 border-t border-border/40">
                                                        <Button
                                                            ...
                                                            className="h-8 px-2.5 rounded-lg bg-muted/60 hover:bg-primary/10 hover:text-primary text-muted-foreground font-black text-[9px] uppercase tracking-wider"
                                                        >
                                                        ...
                                                        <Button
                                                            ...
                                                            className="h-8 px-2.5 rounded-lg bg-muted/60 hover:bg-emerald-500/10 hover:text-emerald-500 text-muted-foreground font-black text-[9px] uppercase tracking-wider"
                                                        >
                                                        ...
                                                        <Button
                                                            ...
                                                            className="h-8 px-2.5 rounded-lg bg-muted/60 hover:bg-blue-500/10 hover:text-blue-500 text-muted-foreground font-black text-[9px] uppercase tracking-wider"
                                                        >
                                                        ...
                                                        <Button
                                                            ...
                                                            className="h-8 w-8 rounded-lg bg-muted/60 hover:bg-accent text-muted-foreground"
                                                        >
                                                            <Download className="h-3.5 w-3.5" />
                                                        </Button>
                                                    </div>
                        ...
                        <div className="mt-2 flex items-center justify-between text-muted-foreground px-4 shrink-0">
                            <div className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-wider">
                                <AlertCircle className={cn("h-2.5 w-2.5", activeSession ? "text-emerald-500" : "text-muted-foreground")} />
                                {activeSession ? "Turno actual en curso" : "Sin turno activo"}
                            </div>
                            <div className="text-[9px] font-bold uppercase tracking-wider bg-muted px-3 py-1 rounded-lg border border-border text-muted-foreground">
