  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent 
          className="max-w-[420px] landscape:max-w-[680px] landscape:md:max-w-[720px] w-[calc(100%-1rem)] sm:w-full p-0 overflow-hidden bg-card border border-border rounded-2xl shadow-2xl max-h-[calc(var(--visual-viewport-height,100dvh)-0.75rem)] sm:max-h-[calc(var(--visual-viewport-height,100dvh)-1rem)] flex flex-col overscroll-contain"
        >
          {/* Header & Total Area Compact */}
          <div className="p-2 sm:p-2.5 pb-0 flex-shrink-0">
            <DialogTitle className="sr-only">
              Cobrar
            </DialogTitle>
            
            {/* Total a Pagar Compact Row */}
            <div className="flex items-center justify-between bg-muted/40 border border-border/80 px-3 py-1 sm:py-1.5 rounded-xl relative overflow-hidden">
              <div>
          ...
          {/* Body Section */}
          <div className="p-2 sm:p-2.5 space-y-2 flex-1 min-h-0 flex flex-col overflow-y-auto">
            <div className="space-y-2 landscape:grid landscape:grid-cols-2 landscape:gap-3 landscape:space-y-0 items-start">
              {/* Columna 1 (Izquierda en horizontal) */}
              <div className="space-y-2 flex flex-col">
                ...
                {/* En modo vertical: CreditInfo y BankAccountsList */}
              ...
              {/* Columna 2 (Derecha en horizontal / Abajo en vertical) */}
              <div className="space-y-2 flex flex-col">
                {/* En modo horizontal: CreditInfo, BankAccountsList o Info de Tarjeta */}
                <div className="hidden landscape:block space-y-2">
                  {paymentMethod === 'credit' && (
                    <div className="p-2.5 bg-muted/30 rounded-lg border border-border">
                      <CreditInfo
                        selectedCustomer={selectedCustomer}
                  ...
                  {paymentMethod === 'card' && (
                    <div className="p-3 bg-muted/20 border border-border/80 rounded-xl flex flex-col items-center justify-center text-center gap-1.5 py-4">
                      <CreditCard className="h-7 w-7 text-primary" />
                      <span className="text-xs font-bold text-foreground">Cobro con Tarjeta</span>
                      <span className="text-[11px] text-muted-foreground">Procese el cobro en el Verifone o terminal y presione Facturar Pago.</span>
                    </div>
                  )}
                </div>

                {(paymentMethod === 'cash' || paymentMethod === 'split') && (
                  <div className="space-y-2 animate-in fade-in slide-in-from-top-1 flex flex-col shrink-0">
                    {paymentMethod === 'split' && (
                    ...
                    <div className="space-y-1 shrink-0">
                      ...
                      <div className="relative group shrink-0">
                        ...
                        <Input
                          ...
                          className="h-9 min-h-[36px] pl-11 pr-3 text-lg font-black bg-background border border-border/80 focus-visible:border-emerald-500 focus-visible:ring-1 focus-visible:ring-emerald-500/20 rounded-xl text-foreground transition-all shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none shrink-0"
                        />
                      </div>
                        ...
                        <div className="flex items-center gap-1 shrink-0 overflow-x-auto py-0.5 scrollbar-none">
                          {localAmount && (
                            <Button
                              ...
                              variant="outline"
                              size="sm"
                              className="h-6 sm:h-7 px-1.5 sm:px-2 rounded-lg border border-border/70 text-[10px] sm:text-[11px] font-bold text-muted-foreground hover:text-destructive hover:border-destructive/40 transition-all shrink-0 gap-0.5"
                              onClick={() => handleAmountChange('')}
                              title="Borrar monto"
                            >
                          ...
                          <Button
                            ...
                            size="sm"
                            className={cn(
                              "h-6 sm:h-7 px-2 rounded-lg border text-[10px] sm:text-[11px] font-bold transition-all shrink-0",
                          ...
                          {suggestedAmounts.map(amt => (
                            <Button
                              ...
                              size="sm"
                              className={cn(
                                "h-6 sm:h-7 px-2 rounded-lg border text-[10px] sm:text-[11px] font-bold transition-all tabular-nums shrink-0",
                                currentReceived === amt
                                  ? "bg-emerald-500/15 border-emerald-500 text-emerald-600 dark:text-emerald-400 font-black shadow-xs"
                      ...
                      {paymentMethod === 'cash' && (
                        <div className="shrink-0 pt-0.5">
                          {currentReceived > fullTotal ? (
                            <div className="py-1.5 px-2.5 rounded-xl bg-emerald-500/15 dark:bg-emerald-950/40 border border-emerald-500/40 flex items-center justify-between shadow-xs transition-all">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  ...
                                  <span className="text-[8px] text-muted-foreground leading-none block">
                                    Entregar al cliente
                                  </span>
                                </div>
                              </div>
                              <div className="text-right">
                                <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 mr-1">RD$</span>
                                <span className="text-lg sm:text-xl font-black text-emerald-600 dark:text-emerald-400 tracking-tight tabular-nums">
                                  {currentChange.toLocaleString('es-DO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </span>
                              </div>
                            </div>
                          ) : currentReceived === fullTotal ? (
                            <div className="py-1.5 px-2.5 rounded-xl bg-muted/40 border border-border/80 flex items-center justify-between text-xs transition-all">
                              <div className="flex items-center gap-1.5 text-muted-foreground font-semibold">
                                <Check className="h-3.5 w-3.5 text-emerald-500 shrink-0" />
                                <span className="text-[9px] font-bold uppercase tracking-wider">Pago Completo (Exacto)</span>
                              </div>
                            ...
                            <div className="py-1.5 px-2.5 rounded-xl bg-amber-500/15 dark:bg-amber-950/40 border border-amber-500/30 flex items-center justify-between text-xs transition-all">
