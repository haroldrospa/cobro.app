...
BEGIN
    ...
    FOR store_record IN SELECT id, name FROM public.stores LIMIT 3 LOOP
        ...
        FOR invoice_type IN SELECT DISTINCT iseq.invoice_type_id FROM public.invoice_sequences iseq WHERE iseq.store_id = store_record.id ORDER BY iseq.invoice_type_id LOOP
            -- Obtener el próximo número (sin incrementar)
            SELECT iseq2.invoice_type_id || '-' || LPAD((iseq2.current_number + 1)::text, 8, '0')
            INTO next_num
            FROM public.invoice_sequences iseq2
            WHERE iseq2.invoice_type_id = invoice_type 
              AND iseq2.store_id = store_record.id;
            
            RAISE NOTICE '  Tipo %: Próximo número será: %', invoice_type, next_num;
        END LOOP;
    END LOOP;
